...
export class CategoriesComponent implements OnInit {
  ...
  submitAdd(){
    ...
    if(this.add){
    this.categoryService.addCategory(this.category).subscribe((category: WeightCategoryModel)=>{
    ...
    this.categoryService.updateCategory(this.category).subscribe(()=>{
      this.categoryService.listCategories().subscribe((data: Array<WeightCategoryModel>)=> {this.categories = data;  
      this.categoryForm.reset();},
      error=> {console.log("error")})},
      error=>{console.log("error")});
  }    
  ...
  removeCategory(category: WeightCategoryModel){
    let remove = confirm("You are about to remove this category! This will remove all related vehicle history.");
    if (remove) {
  ...
  formValidation(){
    ...
    this.categoryForm = new FormGroup({
      ...
      iconId: new FormControl(this.category.iconId, [
        Validators.required
      ])
    });
  }
